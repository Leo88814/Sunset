...
namespace Theater_Admin.Models.Repositories
{
    ...
    public class MovieRepository : IMovieRepository
    {
        ...
        {
            var movie = _db.Movies.AsNoTracking().FirstOrDefault(x => x.Id == id);
            if (movie == null)
                return null;
        ...
        {
            Movie movie = MvcApplication._mapper.Map<Movie>(dto);
        ...
        {
            var movie = _db.Movies.FirstOrDefault(m => m.MovieName == name && m.MovieLanguage == language);
            return movie != null;
        }
    }
}
