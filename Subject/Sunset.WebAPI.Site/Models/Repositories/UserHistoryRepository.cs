    ...
    public class UserHistoryRepository
    {
        ...
        public List<UserHistoryDto> GetUserHistoryByMemberId(string memberId)
        {
            var orders = _context.Orders
                .Where(o => o.MemberId.ToString() == memberId && o.PaymentStatus != "Cancelled") // 過濾掉已取消的訂單
        ...
        public bool CancelOrder(int orderId, int memberId)
        {
            var order = _context.Orders.FirstOrDefault(o => o.Id == orderId && o.MemberId == memberId);
            if (order == null)
            {
                return false;
            }

            order.PaymentStatus = "Cancelled"; // 更新訂單狀態
            _context.SaveChanges();
            return true;
        }
